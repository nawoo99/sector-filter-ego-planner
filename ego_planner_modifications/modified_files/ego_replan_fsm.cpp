...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    end_pt_ = Eigen::Vector3d(1e6, 1e6, 1e6);  // ensure first goal always triggers full view
    ...
    node_->declare_parameter("fsm/z_max", 2.0);
    ...
    node_->get_parameter("fsm/z_max", z_max_);
    ...
    replan_status_pub_ = node_->create_publisher<std_msgs::msg::Bool>("planning/replan_success", 10);
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      {
        // GEN_NEW_TRAJ 중 재트리거: planning 완료 후 자동으로 EXEC_TRAJ 전환되므로
        // 여기서 blocking wait 불필요 → REPLAN으로 즉시 전환
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    double goal_z = std::min(msg->pose.position.z > 0.1 ? msg->pose.position.z : 1.0, z_max_);
    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, goal_z);

    // Only arm full view for genuinely new goals, not for periodic republishes of the same goal
    const bool is_new_goal = (end_wp - end_pt_).norm() > 0.3;
    if (is_new_goal) {
      planner_manager_->grid_map_->armFullViewRecovery("new waypoint received");
    }
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      {
        wp_id_++;
        // Full view scan before planning next leg so side obstacles are visible
        planner_manager_->grid_map_->armFullViewRecovery("waypoint reached");
        planNextWaypoint(wps_[wp_id_]);
      }
