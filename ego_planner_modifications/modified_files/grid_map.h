...
struct matrix_hash {
  std::size_t operator()(T const &matrix) const {
    std::size_t seed = 0;
    for (std::size_t i = 0; i < static_cast<std::size_t>(matrix.size()); ++i) {
...
struct MappingData
{
  ...
  std::vector<uint32_t> flag_traverse_, flag_rayend_;
  uint32_t raycast_num_;
  using AlignedDequeV3i = std::deque<Eigen::Vector3i, Eigen::aligned_allocator<Eigen::Vector3i>>;
  std::queue<Eigen::Vector3i, AlignedDequeV3i> cache_voxel_;
...
private:
  ...
  void applySectorFilter(pcl::PointCloud<pcl::PointXYZ>& cloud_map);
  void applySectorFilterSensorFrame(pcl::PointCloud<pcl::PointXYZ>& cloud_sensor);
...
private:
  ...
  // Embedded sector filter params/state.
  bool sf_enable_{false};
  double sf_min_angle_rad_{-M_PI / 3.0};
  double sf_max_angle_rad_{M_PI / 3.0};
  bool sf_use_stride_{false};
  int sf_stride_{1};
  bool sf_use_voxel_{false};
  double sf_voxel_leaf_{0.10};
  bool sf_adaptive_recovery_{true};
  std::string sf_replan_status_topic_{"/planning/replan_success"};
  int sf_replan_fail_threshold_{3};
  double sf_full_view_hold_sec_{3.0};
  bool sf_replan_burst_enable_{true};
  std::string sf_replan_attempt_topic_{"/planning/replan_attempt"};
  double sf_replan_burst_hold_sec_{0.8};
  double sf_replan_burst_cooldown_sec_{2.0};
  bool sf_auto_disable_on_front_block_{true};
  double sf_front_block_dist_{1.2};
  int sf_front_block_points_threshold_{20};
  double sf_front_block_half_angle_rad_{20.0 * M_PI / 180.0};
  int sf_consecutive_replan_failures_{0};
  int sf_consecutive_replan_successes_{0};
  int sf_return_after_n_success_{3};  // full view → 섹터 복귀에 필요한 연속 성공 횟수
  rclcpp::Time sf_force_full_view_until_;
  rclcpp::Time sf_next_replan_burst_allowed_;
  std::atomic<int> sf_full_view_frames_remaining_{0};  // frame-counter fallback
  bool sf_prev_force_full_view_{false};
};
